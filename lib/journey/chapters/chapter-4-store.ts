/**
 * Chapter IV file-backed store (account-scoped).
 */
...
const DEFAULT_DB_FILE = "chapter-4.json";
const DEFAULT_DB_RELATIVE = `${DEFAULT_DATA_DIR}/${DEFAULT_DB_FILE}`;

const emptyDatabase = (): Chapter4Database => ({
  records: [],
});
...
function normalizeSectionIds(raw: unknown): Chapter4SectionId[] {
  if (!Array.isArray(raw)) {
    return [];
  }
  const seen = new Set<Chapter4SectionId>();
  const next: Chapter4SectionId[] = [];
  for (const entry of raw) {
    if (isChapter4SectionId(entry) && !seen.has(entry)) {
      seen.add(entry);
      next.push(entry);
    }
  }
...
function normalizeReflection(
  raw: StandardsReflectionState | undefined,
  now: string,
...
function normalizePractice(
  raw: StandardsPracticeState | undefined,
  now: string,
...
function normalizeCommitment(
  raw: StandardsCommitmentState | undefined,
  now: string,
...
function normalizeRecord(raw: Chapter4Record): Chapter4Record | null {
...
function normalizeDatabase(raw: Chapter4Database): Chapter4Database {
  const records: Chapter4Record[] = [];
  if (Array.isArray(raw.records)) {
    for (const entry of raw.records) {
      const normalized = normalizeRecord(entry as Chapter4Record);
      if (normalized) {
...
export type Chapter4Store = {
  findChapter4ForUser(userId: string): Promise<Chapter4Record | undefined>;
  saveChapter4(record: Chapter4Record): Promise<Chapter4Record>;
};

export function createFileChapter4Store(options?: {
  dataDir?: string;
  fileName?: string;
}): Chapter4Store {
  ...
  async function readDatabase(): Promise<Chapter4Database> {
    try {
      const raw = await readFile(dbFile, "utf8");
      const parsed = JSON.parse(raw) as Chapter4Database;
  ...
  async function writeDatabase(database: Chapter4Database): Promise<void> {
  ...
  return {
    ...
    saveChapter4(record) {
      return enqueueWrite(async () => {
        ...
        if (!normalized) {
          throw new Error("Invalid Chapter IV payload.");
        }
        const database = await readDatabase();
...
let storeInstance: Chapter4Store | null = null;

function createPostgresChapter4Store(): Chapter4Store {
  const docs = createPostgresChapterDocumentAdapter<Chapter4Record>({
    chapterId: "chapter-4-standards",
    normalize: normalizeRecord,
    invalidMessage: "Invalid Chapter IV payload.",
  });
  return {
    findChapter4ForUser: (userId) => docs.find(userId),
    saveChapter4: (record) => docs.save(record),
  };
}

function createUnconfiguredChapter4Store(): Chapter4Store {
  return {
    findChapter4ForUser: rejectUnconfiguredJourneyStore,
    saveChapter4: rejectUnconfiguredJourneyStore,
  };
}

export function getChapter4Store(): Chapter4Store {
  if (!storeInstance) {
    storeInstance = createJourneyChapterStoreInstance({
      file: () => createFileChapter4Store(),
      postgres: createPostgresChapter4Store,
      unconfigured: createUnconfiguredChapter4Store,
    });
  }
  return storeInstance;
}

export function setChapter4StoreForTests(store: Chapter4Store | null): void {
  storeInstance = store;
}

export function ensureChapter4Record(
  existing: Chapter4Record | undefined,
  userId: string,
): Chapter4Record {
  if (existing) {
    return existing;
  }
  return createEmptyChapter4Record(userId);
}
