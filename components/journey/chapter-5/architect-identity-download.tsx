...
const ARCHITECT_IDENTITY_HREF = "/api/architect/blueprint/architect-identity";

type ArchitectIdentityDownloadProps = {
  locale: Locale;
  className?: string;
};

export function ArchitectIdentityDownload({
  locale,
  className = "bh-cta bh-cta-secondary inline-flex",
}: ArchitectIdentityDownloadProps) {
  ...
  async function download() {
    ...
    setPending(true);
    try {
      const response = await fetch(ARCHITECT_IDENTITY_HREF, {
        method: "GET",
      ...
      anchor.download = BLUEPRINT_EXPORT_FILES.architectIdentityStatement;
