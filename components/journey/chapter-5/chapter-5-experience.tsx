...
import { Chapter5Resources } from "@/components/journey/chapter-5/chapter-5-resources";
import {
  ArchitectCommitmentWork,
  ArchitectPracticeWork,
  ArchitectReflectionWork,
} from "@/components/journey/chapter-5/architect-work";
...
import { getChapter5MediaForSection } from "@/content/journey/chapter-5-media";
import {
...
import { advanceChapter5SectionAction } from "@/lib/journey/chapters/chapter-5-actions";
import {
...
import {
  getChapter5Path,
  getChapter5LuminaDiscussionPath,
...
type Chapter5ExperienceProps = {
  locale: Locale;
  sectionId: Chapter5SectionId;
  record: Chapter5Record;
};

function sectionLabel(locale: Locale, sectionId: Chapter5SectionId): string {
...
export function Chapter5Experience({
  ...
  const welcomeMedia = getChapter5MediaForSection("welcome", locale);

  const reflectionComplete = isArchitectReflectionComplete(reflectionAnswers);
  const practiceComplete = isArchitectPracticeComplete(practice.statement);
  const commitmentComplete = isArchitectCommitmentComplete(commitment);
  ...
  const headingLines = (stage?.heading?.lines ?? []).filter((line) => {
    ...
    return (
      ...
      normalized !== "becoming the architect" &&
      normalized !== "convertirse en architect"
    );
  });

  function continueFrom(section: Chapter5SectionId) {
    setError(null);
    startTransition(async () => {
      await flushJourneyDrafts();
      const result = await advanceChapter5SectionAction({ sectionId: section });
      if (result.status !== "ok") {
        setError(
      ...
      if (result.nextSectionId) {
        router.push(getChapter5Path(locale, result.nextSectionId));
      }
    });
  }
  ...
  return (
    <div className="bh-chapter-1">
      ...
      <ChapterSectionNav
        ...
        items={CHAPTER_5_SECTIONS.map((id) => ({
          id,
          label: sectionLabel(locale, id),
          href: getChapter5Path(locale, id),
        }))}
      />
      <ChapterPauseControl
        locale={locale}
      ...
      {sectionId === "welcome" ? (
        <section
          ...
          aria-labelledby="chapter-5-welcome-heading"
        >
          <h2
            id="chapter-5-welcome-heading"
      ...
      {sectionId === "reflection" ? (
        <section
          className="bh-chapter-1-section"
          aria-labelledby="chapter-5-reflection-heading"
        >
          <h2 id="chapter-5-reflection-heading" className="sr-only">
      ...
      {sectionId === "practice" ? (
        <section
          className="bh-chapter-1-section"
          aria-labelledby="chapter-5-practice-heading"
        >
          <h2 id="chapter-5-practice-heading" className="sr-only">
          ...
          <div className="bh-onboarding-actions">
            <Link
              href={getChapter5Path(locale, "reflection")}
              className="bh-cta bh-cta-secondary"
            >
              {resolveAppShellLabel(locale, copy.back)}
            </Link>
      ...
      {sectionId === "commitment" ? (
        <section
          className="bh-chapter-1-section"
          aria-labelledby="chapter-5-commitment-heading"
        >
          <h2 id="chapter-5-commitment-heading" className="sr-only">
          ...
          <div className="bh-onboarding-actions">
            <Link
              href={getChapter5Path(locale, "practice")}
              className="bh-cta bh-cta-secondary"
            >
              {resolveAppShellLabel(locale, copy.back)}
            </Link>
      ...
      {sectionId === "closing" ? (
        <section
          className="bh-chapter-1-section"
          aria-labelledby="chapter-5-closing-heading"
        >
          <h2
            id="chapter-5-closing-heading"
            className="bh-onboarding-step-title"
          >
          ...
          <div className="bh-onboarding-actions">
            <Link
              href={getChapter5Path(locale, "commitment")}
              className="bh-cta bh-cta-secondary"
            >
              {resolveAppShellLabel(locale, copy.back)}
            </Link>
      ...
      {sectionId === "complete" ? (
        <section
          className="bh-chapter-1-section"
          aria-labelledby="chapter-5-complete-heading"
        >
          <h2
            id="chapter-5-complete-heading"
            className="bh-onboarding-step-title"
          >
          ...
          <Chapter5Resources locale={locale} />
          <div className="bh-chapter-1-complete-actions">
            ...
            <Link
              href={getChapter5LuminaDiscussionPath(locale)}
              className="bh-cta bh-chapter-1-complete-action"
            >
