...
const EXPANSION_PLAN_HREF = "/api/architect/blueprint/expansion-plan";

type ExpansionPlanDownloadProps = {
  locale: Locale;
  className?: string;
};

export function ExpansionPlanDownload({
  locale,
  className = "bh-cta bh-cta-secondary inline-flex",
}: ExpansionPlanDownloadProps) {
  ...
  async function download() {
    ...
    setPending(true);
    try {
      const response = await fetch(EXPANSION_PLAN_HREF, {
        method: "GET",
      ...
      anchor.download = BLUEPRINT_EXPORT_FILES.expansionPlan;
