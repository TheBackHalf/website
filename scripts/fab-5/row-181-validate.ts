...
type Verdict = "PASS" | "FAIL" | "NOT_RUN" | "NOT_IN_SCOPE";
...
const STATUS_FILE = "ops/fab-5/runs/aos-engineering-status/al-181-correction.json";
...
function recordNotInScope(row: Omit<TestRow, "verdict" | "pass">): void {
  tests.push({ ...row, verdict: "NOT_IN_SCOPE" });
}
    ...
    tables: [
      {
        ...
        rows: [
          [
            ...
            JSON.stringify({ stripeCheckoutSessionId: "cs_test_al181_restore", amountCents: 50000 }),
            true,
          ],
          [
            "evt-al181-2",
      ...
      empty("marketing_kpi_events", idColumns),
      {
        ...
        rowCount: 1,
        rows: [["kpi-al181-1", now, 50000, "cs_test_al181_restore", true]],
      },
      ...
      empty("bh_auth_users", idColumns),
      empty("bh_auth_consents", idColumns),
      empty("bh_durable_documents", [
        { name: "collection", udt: "text", nullable: false, primaryKey: true },
        { name: "doc_id", udt: "text", nullable: false, primaryKey: true },
      ]),
      empty("bh_lumina_conversations", idColumns),
      empty("bh_lumina_memories", [
...
async function main(): Promise<void> {
  ...
  const firstEvent = await trackProductEvent({
    ...
    userId: user.id,
    payload: { stripeCheckoutSessionId: sessionId, offerId: "blueprint", amountCents: 50000 },
    attribution,
  });
  ...
  record({
    ...
    pass:
      ...
      kpiFirst.purchase.amountCents === 50000 &&
      kpiFirst.purchase.stripeCheckoutSessionId === sessionId,
  });

  const luminaSrc = await readFile("lib/lumina/store.ts", "utf8");
  const progressSrc = await readFile("lib/journey/progress/store.ts", "utf8");
  const billingSrc = await readFile("lib/billing/store.ts", "utf8");
  record({
    id: "S7",
    area: "saving",
    name: "Launch-critical stores select durable Postgres in production",
    expected:
      "Lumina, Journey progress, and billing projection select supabase_postgres when configured and fail closed on hosted production",
    actual: `luminaPg=${luminaSrc.includes("createPostgresLuminaStore")}; journeyPg=${progressSrc.includes("createPostgresJourneyProgressStore")}; billingPg=${billingSrc.includes("createPostgresBillingStore")}; failClosed=${luminaSrc.includes("unconfigured_production")}`,
    pass:
      luminaSrc.includes("createPostgresLuminaStore") &&
      progressSrc.includes("createPostgresJourneyProgressStore") &&
      billingSrc.includes("createPostgresBillingStore") &&
      luminaSrc.includes("unconfigured_production"),
  });
  ...
  record({
    ...
    pass: restoredEvent?.userId === user.id && restoredKpi?.amountCents === 50000,
  });
  ...
  recordNotInScope({
    id: "X4",
    area: "deletion",
    name: "Account erasure is out of launch scope",
    expected:
      "Launch participant copy states account deletion is not available yet. Row 181 tests deletion of tokens, Lumina memory, and targeted events — not GDPR erasure.",
    actual: `deleteUser=${hasMethod(auth, "deleteUser")}; deleteAccount=${hasMethod(auth, "deleteAccount")}; citations=content/i18n/dictionaries/en.ts account deletion which is not available yet`,
  });
  ...
  remainingRisks.push(
    "Production WAL/PITR dashboard proof and a live logical export from Production remain Founder-held unless the CRON_SECRET-gated /api/fab-5/aos/backup-probe is invoked on Vercel Production. This workstation has no POSTGRES_URL.",
  );
  remainingRisks.push(
    "Stripe remains the payment system of record. Postgres billing tables are a webhook/entitlement projection; restoring them is not a Stripe ledger restore.",
  );
  remainingRisks.push(
    "Account erasure is OUT OF LAUNCH SCOPE (participant copy: account deletion is not available yet). Deletion tests cover tokens, Lumina memory clear, and targeted events only.",
  );
  ...
  const evidence = {
    ...
    productMarketingLegalChanged: true,
    founderAcceptanceRecorded: false,
    rowMarkedComplete: false,
    overall,
    residualRisk: !production.attempted ? "HIGH" : failed.length > 0 ? "HIGH" : "MEDIUM",
    nextStatus,
