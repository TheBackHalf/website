...
export const metadata: Metadata = createArchitectPageMetadata("es", "settings");

export default async function EsArchitectSettingsPage() {
  const profile = await getArchitectProfileForSession();

  if (!profile) {
    redirect(`${getLoginPath("es")}?next=/es/architect/settings`);
  }
