...
export const metadata = createUnsubscribeMetadata("es");
...
export default async function UnsubscribePageEs({
  searchParams,
}: UnsubscribePageProps) {
  const params = await searchParams;
  if (!params.token) {
    const status = params.status === "invalid" ? "invalid" : "missing";
    return <UnsubscribePageView locale="es" status={status} />;
  }

  const result = await processUnsubscribeRequest(params.token, "unsubscribe_page");
  if (result.status === "invalid") {
    return <UnsubscribePageView locale="es" status="invalid" />;
  }
  return (
    <UnsubscribePageView
      locale="es"
