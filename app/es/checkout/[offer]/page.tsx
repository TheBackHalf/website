...
export async function generateMetadata({
  ...
  const meta = getDictionary("es").metadata.checkout;
...
export default async function EsCheckoutOfferPage({ params }: PageProps) {
  ...
  if (!session) {
    redirect(
      `${getLoginPath("es")}?next=${encodeURIComponent(`/es/checkout/${offerId}`)}`,
    );
  }

  const user = await getAuthStore().findUserById(session.sub);
  if (user && user.ageEligible === false) {
    redirect(getNotEligiblePath("es"));
  }
  if (!accountIsAgeEligible(user)) {
    redirect(getEligibilityPath("es", `/es/checkout/${offerId}`));
  }
      ...
      <main id="checkout-main" className="min-h-screen bg-bh-cream text-bh-ink">
        <PageHero locale="es">
          ...
          <SectionHeading as="h1" className="mt-6 text-4xl md:text-6xl lg:text-7xl">
            {translate("es", dictionary.checkout.catalogTitle)}
          </SectionHeading>
        </PageHero>
        ...
        >
          <CheckoutOfferForm offerId={offerId} locale="es" />
        </SectionShell>
