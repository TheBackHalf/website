...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const asset = getFounderLaunchA11yAsset("es", id);
  return {
...
export default async function SpanishFounderTranscriptPage({ params }: PageProps) {
  const { id } = await params;
  return <FounderTranscriptPage locale="es" id={id} />;
}
