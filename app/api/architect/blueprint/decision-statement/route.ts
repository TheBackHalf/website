...
export async function GET(request: Request) {
  ...
  const printPath = `/blueprint/print/artifacts/decision-statement?architectId=${encodeURIComponent(actor.user.id)}`;

  try {
    const pdf = await renderAuthenticatedPrintPdf({ request, printPath });
    await download.completed();
    return pdfAttachmentResponse(pdf, "back-half-decision-statement.pdf");
  } catch (error) {
    await download.failed();
    console.error("Personalized Decision Statement PDF generation failed:", error);
    return NextResponse.json(
      { error: "Unable to generate personalized Decision Statement." },
      { status: 500 },
    );
  }
}
